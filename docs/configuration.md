...
Codex records directory trust once per repository root in the captain's own `${CODEX_HOME:-~/.codex}/config.toml`, so an authorized project or secondmate-home intake registers that root through `bin/fm-codex-trust.sh` rather than leaving the first worker on the directory dialog; [runtime backend verification](verification/runtime-backends.md#codex-directory-trust) owns that evidence and the helper's header owns its limits.
